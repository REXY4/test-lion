...
const Home: React.FC<Props> = ({user,modalAuth,  getAllProduct, product, login, AddCart, cart}) => {
            ...
            <Row className="mt-5">
                <Col md={6}>
                   <div className="mt-5">
                    ...
                    <p className={styles.paragraft}>Lorem ipsum dolor sit amet consectetur adipisicing elit. Expedita recusandae debitis, sint molestiae tempore repudiandae minus explicabo sed dolor obcaecati? Iusto obcaecati eius veniam officiis nobis rerum ab impedit vel?</p>
                  </div>
                </Col>
            </Row>
            <Row className="mt-5">
                {product.slice(currentPage, previousPage).map((item:any)=>{
                  return (
                    <Col key={item.name} md={3} className={`mb-4 ${styles["card-product"]}`}>
                    <CardProduct 
                    addCart={AddCart}
                    image={item.image} 
                    name={item.name}
...
export default connect(mapStateToProps,{getAllProduct, login, AddCart})(Home);
